...
private:
    ...
    auto checkFileType(std::istream &input) -> bool ;
public:
    WAVFile()  ;
    ...
    auto timeForSample(std::uint32_t sample) const -> double ;
    auto sampleForTime( double time) const -> std::uint32_t ;
    
    auto nextSample() const -> std::uint32_t ;
    auto nextTime() const -> double ;
    
    auto setNextTime(double time) -> bool ;
    auto setSample(std::uint32_t sample) -> bool ;
